...
class EngineFacade(Facade):
    ...
    def set_cam_pos(self, pos): return base.camera.set_pos(pos)
    ...
    def toggle_pause(self, show_frm=True): return self.pause.logic.toggle(show_frm)
    ...
    def set_dir_lgt(self, col, direction): return self.shader_mgr.set_dir_lgt(col, direction)
    def clear_lights(self): return self.shader_mgr.clear_lights()
    def toggle_shader(self): return self.shader_mgr.toggle_shader()
    def set_resolution(self, res, fullscreen=None): return self.gui.set_resolution(res, fullscreen)
    def toggle_fullscreen(self): return self.gui.toggle_fullscreen()
    def send(self, msg): return self.lib.send(msg)
    def do_later(self, time, meth, args=None): return self.lib.do_later(time, meth, args)
    def add_task(self, mth, priority=0): return self.lib.add_task(mth, priority)
