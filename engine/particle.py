...
class Particle(GameObject):
    ...
    def __init__(self, parent, pos, hpr, texture, tot_time, npart, color=(1, 1, 1, 1), ampl=pi/6, ray=.5, rate=.0001, gravity=-.85, vel=3.8, part_time=None):
        if not self.eng.lib.version.startswith('1.10'): return
        GameObject.__init__(self)
        if part_time is None: part_time = tot_time
        self.__part = LibParticle(parent, pos, hpr, texture, part_time, npart,
                                  color, ampl, ray, rate, gravity, vel)
