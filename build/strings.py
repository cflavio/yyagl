...
def __prepare(lng_base_dir, lng, appname):
    ...
    if not exists('assets/po/' + lng + '.po'):
        ...
        list(map(
            lambda line: __fix_line(line, lng_dir, appname), lines_to_fix))
...
def __merge(lng_dir, lng_code, appname):
    ...
    remove(
        '{lng_dir}{appname}merge.po'.format(lng_dir=lng_dir, appname=appname))


def __postprocess(lng_dir, lng_code, appname):
    # unused lng_dir, appname
