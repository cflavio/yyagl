...
def bld_p3d(target, source, env):
    ...
    seds = [('version', ''), ('AppName', ''), ('appname', ''), ('yyaglpath', 'g')]
    seds = ["-e 's/<%s>/{%s}/%s'" % (sel[0], sel[0], sel[1]) for sel in seds]
    tmpl = 'sed %s {curr_dir}template.pdef > {appname}.pdef' % ' '.join(seds)
    cmd = tmpl.format(
        version=ver, AppName=env['APPNAME'].capitalize(), appname=env['APPNAME'],
        curr_dir=fpath + '/', yyaglpath=yyaglpath)
    system(cmd)
...
def __bld(suff, env):
    ...
    p3d_src = p3d_src_tmpl.format(
        src_dir=bld_dpath, AppName=env['APPNAME'].capitalize(), version=ver, suff=suff)
