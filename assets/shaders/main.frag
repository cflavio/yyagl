...
void toon(int lgt_idx, out vec3 amb_diff, out vec3 spec) {
  ...
  spec *= step(.0, s_dot_n);
  if (lgt.exp == .0) return;  // point light or directional, not a spotlight
  float ang = acos(dot(-s, lgt.dir));
  float cutoff = radians(clamp(lgt.cutoff, 0, 90));
  float col_val = toon_val(pow(dot(-s, lgt.dir), lgt.exp));
  float spot_factor =  col_val * step(ang, cutoff);
...
vec4 vsaturate(vec4 v) {
  return vec4(clamp(v[0], .0, 1.0), clamp(v[1], .0, 1.0),
              clamp(v[2], .0, 1.0), clamp(v[3], .0, 1.0));
}
