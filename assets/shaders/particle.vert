...
void main() {
    ...
    vec4 vel = imageLoad(curr_vel, gl_VertexID);
    time = osg_FrameTime - start_time;
    ptime = 0;
    vec4 pos = imageLoad(curr_pos, gl_VertexID);
    ...
    imageStore(curr_pos, gl_VertexID, pos);
    imageStore(curr_vel, gl_VertexID, vel);
}
